...
- [x] **Official Releases**
  ...
  - `v1.5.0`: Minor feature release delivering Phone-Initiated Pull-to-Refresh Synchronization with Wear OS Wake-Up (US 5.4 / Issue #23), Bidirectional Sync Resiliency Hardening, and Wear OS Workout Flow UX Enhancement with "Übung beenden" ("Finish Exercise") action (code 8).
  - `v1.5.1`: Patch release hardening the Wearable Data Layer sync engine against stalls and race conditions, and redesigning mobile weight stepper controls into a 2-row layout with dynamic machine increment adaptation and anti-wrapping safeguards (code 9).
  - `v1.6.0`: Minor feature release delivering Brand Redesign with Minimalist AI Vector Mark & Native App Icons (Adaptive & Round) across Mobile and Wear OS, In-Memory End-to-End Synchronization Test Suite with On-Demand CI Trigger, and shared SyncIngestionEngine refactoring (code 10).
- [x] **Brand Redesign & Minimalist AI App Icons (`:mobile`, `:wear`, `documentation`)**
  - Redesigned visual branding: Replaced old heavy skeuomorphic lock logo with an ultra-minimalist geometric AI mark fusing an upward lifting chevron arrow and a continuous circular vault loop in luminous electric cyan on deep obsidian background (`#11141B`).
  - Mobile App Icons (`:mobile`): Generated adaptive icon XMLs (`res/mipmap-anydpi-v26/ic_launcher.xml`, `ic_launcher_round.xml`), adaptive foregrounds (`ic_launcher_foreground.png`), background color resource (`ic_launcher_background.xml`), and full density legacy raster mipmaps (`mdpi`, `hdpi`, `xhdpi`, `xxhdpi`, `xxxhdpi`).
  - Wear OS App Icons (`:wear`): Generated circular launcher icons (`ic_launcher_round.png`), standard launcher icons (`ic_launcher.png`), adaptive XMLs, and full density mipmaps (`mdpi` 48px to `xxxhdpi` 192px), giving the smartwatch launcher a dedicated native round icon.
  - Manifest Wiring: Updated `AndroidManifest.xml` in `:mobile` and `:wear` from generic `@android:drawable/sym_def_app_icon` to `@mipmap/ic_launcher` and `@mipmap/ic_launcher_round`.
  - Documentation Assets: Updated `documentation/assets/lockerlift_app_icon.png` and `lockerlift_app_icon.jpg` with high-resolution 512x512 assets for README and store listings.
- [x] **In-Memory End-to-End Synchronization Test Suite with On-Demand CI Trigger (`:core:sync`, `:core:database`, `:mobile`, `:wear`, `.github`)**
  - `SyncIngestionEngine` (`:core:sync`): Extracted shared, single-source-of-truth ingestion engine for master equipment catalog (`ingestEquipmentCatalog`), master workout templates (`ingestWorkoutTemplates`), bidirectional workout session ingestion (`ingestWorkoutPayload`), ACK handling (`handleWorkoutAck`), and deletion propagation (`handleWorkoutDelete`). Eliminates code duplication between `MobileDataLayerListenerService` and `WearDataLayerListenerService`.
  - `LockerLiftDatabase` (`:core:database`): Added `createInMemory(context)` companion helper for fast, headless in-memory Room SQLite database creation without SQLCipher native library requirements.
  - `MobileDataLayerListenerService` (`:mobile`) & `WearDataLayerListenerService` (`:wear`): Refactored message handling, master data ingestion, and session processing to delegate cleanly to `SyncIngestionEngine`.
  - `InMemoryE2eSyncTest` (`:core:sync`): 7 comprehensive Robolectric-based in-memory end-to-end integration tests:
    1. `testWorkoutSessionSync_fromWatchToPhone_persistsFullGraphAndAcknowledges`: Full session graph transfer, machine creation, set persistence, and ACK loopback.
    2. `testWorkoutSessionSync_fromPhoneToWatch_persistsFullGraphAndAcknowledges`: Multi-machine session sync from phone to watch with ACK return and sync queue purging.
    3. `testMasterDataSync_catalogAndTemplatesReplicatedToWatchPreservingOrder`: Catalog & template replication preserving exact exercise order.
    4. `testMachineReconciliation_whenWatchAndPhoneHaveSameMachineNameWithDifferentIds_preventsConstraintCrashAndRemaps`: Ad-hoc machine collision reconciliation remapping instances to local IDs, preventing SQLite `FOREIGN KEY` / unique constraint crashes.
    5. `testHistoricalDataParity_forDoubleProgressionAfterSync`: Verifies `getLastCompletedSetsForMachine` returns identical historical sets on both devices after sync.
    6. `testZombieWorkoutRejection_onMobileDoesNotResurrectDeletedSession`: Rejects resurrecting locally deleted sessions pending delete sync.
    7. `testWorkoutDeletionSync_removesSessionAndQueueAcrossDevices`: Deletion propagation purging session and queue items across devices.
  - On-Demand CI Execution & Selective Gradle Filtering: E2E sync suite is excluded by default from standard local builds and standard push/PR CI runs via Gradle `tasks.withType<Test>` filtering and JUnit `Assume.assumeTrue`.
  - GitHub Actions Workflow (`build-and-test.yml`): Added `workflow_dispatch` boolean input `run_e2e_sync` ("In-Memory E2E Synchronisations-Testsuite explizit ausführen") enabling manual on-demand execution via `-PrunE2eSync=true`.
- [x] **US 5.4: Phone-Initiated Pull-to-Refresh Synchronization with Wear OS Wake-Up & Sync Resiliency (`:mobile`, `:wear`, `:core:sync`)**
  - `HistoryScreen` & `TemplateListScreen` (`:mobile`): Implemented pull-to-refresh gesture via Material 3 `PullToRefreshBox` with native refresh spinner and non-blocking Snackbar feedback.
  - `SyncPullRefreshCoordinator` (`:core:sync`): State machine coordinating master data push (`pushAllMasterData`), companion reachability check, `/sync/request_flush` wake-up dispatch, reactive completion listening via `SyncEventBus`, and 8-second safe timeout fallback.
  - `WearDataLayerListenerService` (`:wear`): Immediate offline queue flush upon receiving `/sync/request_flush` without periodic WorkManager constraints, dispatching completion message `/sync/flush_completed` with transferred workout count.
  - Synchronizer & Store-and-Forward Fixes: Fixed missing `SyncQueueWorker` enqueue in `ActiveWorkoutScreen.kt` upon workout completion; eliminated sender-side channel abortion in `sendWorkoutPayloadViaChannel`; added fast atomic `MessageClient` payload transfer fallback (`PATH_WORKOUT_MESSAGE`); updated manifests to declare all intent-filter data path prefixes (`/workout_delete`, `/workout_payload_message`, `/sync/request_flush`, `/sync/flush_completed`, `/sync/request_master_data`).
  - Unit Tests: `SyncPullRefreshCoordinatorTest.kt` covering companion reachability, master data push, flush completion response, timeout dismiss, and protocol path contracts.
- [x] **Wear OS Tracking UX Enhancement: "Übung beenden" ("Finish Exercise") (`:wear`)**
  - `RepsWeightInputScreen` (`:wear`): Replaced unintuitive "Cancel" / "Abbrechen" button with prominent "Finish Exercise" / "Übung beenden" button (`btn_finish_exercise`) when sets have already been completed (`setNumber > 1` and `!isEditing`). First set retains "Cancel" ("Abbrechen") for explicit discarding.
  - `RestTimerScreen` (`:wear`): Added dedicated `onFinishExercise` callback and `btn_finish_exercise` action alongside "Next Set" and "Next Exercise".
...
- [x] **Material 3 Theming, Edge-to-Edge & Appbar Placement Fix (`:mobile`)**
  ...
  - Modern Material 3 Theme (`LockerLiftTheme`): Implemented dynamic color adaptation on Android 12+ (`dynamicDarkColorScheme` / `dynamicLightColorScheme`), modern dark & light color schemes with clean athletic slate/indigo tones, and automatic dark mode support (`isSystemInDarkTheme()`).
