...
object WearWorkoutLogic {
    ...
    fun calculateNextWeight(currentWeight: Float, increment: Float, shouldProgress: Boolean): Float {
        return if (shouldProgress) currentWeight + increment else currentWeight
    }
     ...
     * 1. If the current session already has sets logged for this station, use the last logged set.
     * 2. If this is set 1 and no sets are logged yet, use the first set from the last completed session.
     * 3. Otherwise, use default fallback values.
     */
    fun determinePrefillWeightAndReps(
        ...
        if (lastCurrentSet != null) {
            return Pair(lastCurrentSet.weightKg, lastCurrentSet.reps)
        }
